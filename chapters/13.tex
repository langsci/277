...
\REF{ex:key:13.12}. This is exactly what is expected, if CP is a
...
\ea%19
    \label{ex:key:13.19}\glsunset{CDAP}
...
\REF{ex:key:13.20} are set positively, [Yes, \gls{CDAP}, Yes \gls{DAP}]. As a result, the
...
A particularly clear case of case stacking, discussed in \citet{Baker2015}, is
seen in Cuzco Quechua, where an NP can get genitive\is{genitive case} case as
the subject of a nominalized clause (i.e., as possessor of an NP), but then
...
this means that \ili{Greek} is a \gls{CDAP}:no language like \ili{Bantu} or
...
\gls{ECM}\is{exceptional case marking} that \ili{Greek} behaves like a \gls{CDAP}:no
...
the \is{topic} values the uD-feature of T, where the valuation consists of uD
copying the referential index of the \is{topic}.
