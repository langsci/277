...
structural case is assigned by the principles of \isi{dependent case} assignment
...
into \isi{Agree} with it, see \eqref{ex:key:13.1} from \citet{Baker2015}, but cf.
\textcite{Sigurdsson2000}, who argues for a \emph{v}P based approach.

\ea\label{ex:key:13.1} Overt NP X has nominative\is{nominative case} case if and only if exactly
...
The squib is structured as follows: in~\Cref{sec:key:13.2,sec:key:13.3}, we
present Baker’s criteria to determine the two modes of Case assignment,
\isi{Agree} vs.\ \isi{dependent case}. In~\Cref{sec:key:13.4}, we apply these
criteria to \ili{Greek}.  In~\Cref{sec:key:13.4}, we address the issue of
...
\REF{ex:key:13.4}, we find an overt nominative\is{nominative case} subject when
the verb bears agreement, but not otherwise.

\ea%4
    \label{ex:key:13.4} \ili{Sakha} \parencite[29]{Baker2015}
	\ea
...
\ea%5
    \label{ex:key:13.5}\ili{Sakha} \parencite[30]{Baker2015}
    \sn[*]{
    \gll Masha cej ih-er-e caakky\\
...
\REF{ex:key:13.6}.\is{relative clauses}

\ea%6
    \label{ex:key:13.6}\ili{Sakha} \parencite[30]{Baker2015}
    \sn
...
\ea%7
    \label{ex:key:13.7}\ili{Sakha} \parencite[30]{Baker2015}
    \sn
...
\REF{ex:key:13.6} suggests that if a different head agrees with the subject in
...
subject bears nominative\is{nominative case}. In \REF{ex:key:13.6}, it is the
...
null, see \REF{ex:key:13.8}:

\ea%8
    \label{ex:key:13.8}\ili{Sakha} \parencite[30]{Baker2015}
    \sn
...
as in \eqref{ex:key:13.9}, a fact indicating that there can be no nominative\is{nominative case} in the
absence of agreement in this language:\is{relative clauses}

\ea%9
    \label{ex:key:13.9}\ili{Sakha} \parencite[30]{Baker2015}
    \sn[*]{
    \gll  Masha cej ih-er caakky.\\
...
nominative, (\ref{ex:key:13.10}a), the passive\is{passive} verb must \isi{Agree} with it; if it
is not nominative\is{nominative case}, (\ref{ex:key:13.10}b), then the passive\is{passive} verb cannot agree
with it:

\ea%10
    \label{ex:key:13.10}\ili{Sakha} \parencite[32]{Baker2015}
	\ea
...
Agree.\footnote{As pointed out by an anonymous reviewer, \citet{LevPre2015}
    argue that these facts are equally consistent with the view that nominative\is{nominative case}
    is the unmarked case in the language, and that agreement targets only NPs
with nominative\is{nominative case} case; We will come back to the issue of agreement targeting \Nom{}
DPs.}
...
\ea%11
    \label{ex:key:13.11}\ili{Sakha} \parencite[112]{Baker2015}
    \sn
...
\REF{ex:key:13.12}. This is exactly what it is expected, if CP is a
phase\is{phases} in
\citegen{Chomsky2001} sense:

\ea%12
    \label{ex:key:13.12}\ili{Sakha} \parencite[113]{Baker2015}
    \sn
...
\ea%13
    \label{ex:key:13.13}\ili{Sakha} \parencite[114]{Baker2015}
    \sn
...
\ea%14
    \label{ex:key:13.14}\ili{Sakha} \parencite[115]{Baker2015}
    \sn
...
case\is{dependent case} assignment are \isi{phases} in the sense of \citet{Chomsky2001}.
...
\ea%15
    \label{ex:key:13.15}\ili{Oromo} \parencite[99]{Baker2015}
	\ea
...
\ea%16
    \label{ex:key:13.16}\ili{Ingush} \citep[263]{Nichols2011}
    \sn
...
\ea%17
    \label{ex:key:13.17}\ili{Ingush} \parencite[551, 550]{Nichols2011}
	\ea
...
\ea%18
    \label{ex:key:13.18}\ili{Burushaski} \parencite[44]{Baker2015}
	\ea
...
according to \citet{Baker2008}: First, there are many \ili{Bantu} languages that
...
\ea%21
    \label{ex:key:13.21}\ili{Kinande} \citep[158]{Baker2008}
	\ea
...
Second, many \ili{Indo-European} languages systematically obey \eqref{ex:key:13.19} but
...
Third, there are languages such as \ili{Turkish} where both \eqref{ex:key:13.19} and
\REF{ex:key:13.20} are set positively, [Yes, CDAP, Yes DAP]. As a result, the
finite verb only agrees with nominative\is{nominative case} DPs, but only in SOV orders, not in
...
control construction. As shown in (\ref{ex:key:13.22}b), object \isi{control}
...
\eqref{ex:key:13.22}, does not arise in \ili{Sakha}, as far as he knows.}

\ea%22
    \label{ex:key:13.22}\ili{Greek}
    \ea[]{
...
and the predicate must \isi{Agree} in \ili{Greek}, we see that no matter what the case of
the subject is (\Nom{} or \Acc), the embedded predicate always bears nominative\is{nominative case}
(data from \citealt{KotzPapa2007}):

\ea%24
\label{ex:key:13.24}\ili{Greek} \parencite{KotzPapa2007}
	\ea
...
seen in Cuzco Quechua, where an NP can get genitive\is{genitive case} case as the subject of a
nominalized clause (i.e., as possessor of an NP), but then move up into a
higher clause and get \isi{accusative case} by being c-commanded by the subject on
top of its genitive\is{genitive case} case.

\ea%25
\label{ex:key:13.25}\ili{Cuzco Quechua} \parencite[116]{Baker2015}
	\ea
...
\ea\label{ex:key:13.27}\ili{Greek}
    \ea[*]{
...
In these contexts, the nominative\is{nominative case} can appear in the embedded clause, in spite
of the absence of T. In this type of construction, similar to the languages
discussed in section 2, we have two verbs that \isi{Agree} with one nominative\is{nominative case}
obligatorily, a \gls{LDA} phenomenon, see \textcite{AleAnaIorMar2012} for
detailed argumentation and arguments that this is not a covert \isi{raising}
construction but genuine \gls{LDA}:\newpage

\ea%28
\label{ex:key:13.28}\ili{Greek} (AAIM 2012: (36))
    \sn
...
\ea%29
    \label{ex:key:13.29}\ili{Greek} (AAIM 2012: (41), (63))
...
correspondence between nominative\is{nominative case} case and verbal agreement (a single
nominative and many full agreements can co-occur) and that nominative\is{nominative case} is
realized in environments where \isi{Agree} with a nominative\is{nominative case} assigning head does not
take place (in the ECM,\is{exceptional case marking} Raising\is{raising} and \gls{LDA} constructions with embedded T lacking
semantic and morphological tense discussed above). These phenomena are
reminiscent of the ones attested in \ili{Burushaski}, \ili{Tamil},
\ili{Ingush}, \ili{Tsez}, which have
been analyzed by Baker in terms of unmarked nominative\is{nominative case}
(see~\Cref{sec:key:13.3}).

Further evidence for unmarked nominative\is{nominative case} in \ili{Greek} is drawn from a series of
environments where nominative\is{nominative case} surfaces in the absence of agreement. For
example:

(i) Nominative assigned in the absence of agreement; \ili{Greek} free-adjunct
...
Our conclusion leads to the following question: if nominative\is{nominative case} is unmarked, then
this means that \ili{Greek} is a CDAP–No language like \ili{Bantu} or
\ili{Burushaski}. But then
why does the inflected verb in \ili{Greek} only \isi{Agree} with nominative\is{nominative case} NPs and never
...
regardless of the case of the subject (ergative or nominative\is{nominative case}) and regardless
of where the thematic subject is placed.

Note that, as is well-known, the nominative\is{nominative case} NP does not need to be dislocated
to Spec,TP in \ili{Greek}, i.e.\ \ili{Greek} clearly qualifies as a DAP-No language
(\citealt{AleAna1998}, i.a.):\newpage

\ea%32
\label{ex:key:13.32}\ili{Greek}
	\ea
...
Crucially, verbal agreement is always with the nominative\is{nominative case} argument and never
with e.g.\ a higher locative or dative\is{dative case} argument. \citet{Anagnostopoulou1999}
provides evidence that dative\is{dative case} experiencers in \ili{Greek} have subject status with
...
nevertheless, verbal agreement in this case is with the nominative\is{nominative case} and not with
the dative\is{dative case} argument:
...
quirky subject constructions in \ili{Greek}, unlike e.g.\ Icelandic, where the verb is
not allowed to \isi{Agree} with a nominative\is{nominative case} object if this is first or second person
...
experiencer argument agreement in person with the nominative\is{nominative case} argument is
possible:

\ea%35
\label{ex:key:13.35}\ili{Greek}
    \sn
...
\ea%36
    \label{ex:key:13.36}\ili{Icelandic}
    \ea[]{
...
\gls{ECM}\is{exceptional case marking} that \ili{Greek} behaves like a CDAP–No
language, we also have evidence that inflected verbs agree (fully) with
nominative\is{nominative case} arguments, just as in many Indo-European
...
\isi{φ-features} on T, according to the hypothesis in \eqref{ex:key:13.37} (see
...
\REF{ex:key:13.27}--\REF{ex:key:13.29} are not deleted by entering \isi{Agree} with
\Nom{} arguments, and can thus form an \gls{LDA} chain with the \isi{φ-features} of
the higher T:

\ea%38
    \label{ex:key:13.38}
    \glspl{NSL} have T with interpretable \isi{φ-features} which are not deleted after
    checking, thus being able to form Long Distance chains via \isi{Agree} (cf.
    \citealt{Ura1994}).
\z
...
Nom in \glspl{NSL} and not \Nom{} assigned by \isi{Agree} in such a theory. In other words, the
prediction of hypothesis \eqref{ex:key:13.37} is the unmarked status of nominative\is{nominative case}
in NSLs.

This prediction seems to be borne out in \ili{Greek} and at least in \ili{Romanian}, among
other NSLs. \ili{Romanian} like \ili{Greek} has \gls{LDA} (AAIM 2012): as shown in
\REF{ex:key:13.40}, the in situ DP subject obligatorily agrees with both the
matrix and the lower verb in person and number, just like \ili{Greek}:

\ea\label{ex:key:13.40}\ili{Romanian}
    \sn
...
\emph{In situ} subjects take narrow scope with respect to \isi{raising} verb and
matrix negation, as shown in \eqref{ex:key:13.41} (compare to \REF{ex:key:13.29}
...
\ea%41
    \label{ex:key:13.41}\ili{Romanian}
...
Like \ili{Greek}, \ili{Romanian} allows nominative\is{nominative case} \emph{in gerunds}:

\ea\label{ex:key:13.42}\ili{Romanian} \citep{Alboiu2009}
    \sn
...
nominative in particular cases, for instance, \ili{Bantu} languages show agreement
between the finite verb and whatever precedes it (locatives, objects etc.),
while \ili{Burushaski} T agrees indiscriminately with both ergative and
absolutive subjects.\is{agreement!subject agreement} \ili{Greek} instantiates
...
(113d)]{Baker2008}. Multiple agreement in \ili{Greek} and \ili{Romanian} suggests
...
Nevertheless, agreement can never trace genitive\is{genitive case} DP
...
\REF{ex:key:13.39}. Once its \isi{φ-features} are valued, the lower T in \REF{ex:key:13.39}
...
a \Nom{} DP, similarly to e.g.\ \ili{English} and Sakha. This is due to the fact that in
...
\ili{Sakha} and \ili{English}. \ili{Greek} behaves similarly to \ili{Oromo},
\ili{Ingush}
and \ili{Tsez} in showing multiple fully inflected for person and number verbal heads
agreeing with a single Nom DP (\glsunset{LDA}\gls{LDA}). This is due to the
fact that T in pro-drop \ili{Greek} has [+interpretable] φ-features which do
not have to be licensed via \isi{Agree} with a \Nom{} DP, and, concomitantly,
...
This view on pro is very close to ideas in \citet{Holmberg2010} and
...
which is valued by the \is{topic}, the result is a definite null subject\is{null
subjects}
construction. The chain is pronounced in form of an affix on the verb.
Specifically, in Holmberg's system the index-sharing relationship between the
null pronoun and the null Topic crucially involves T: the \is{topic} values the
uD-feature of T, where the valuation consists of uD copying the referential
index of the \is{topic}.
...
while they share φ-feature\is{φ-features} values as a result of \isi{Agree}, they do not form a
chain, and consequently the lexical subject is spelled out and pronounced. In
our analysis, though valuation is necessary, the additional layer of [+
interpretable] features leads to a situation, according to which Nom Case will
not be assigned as a result of \isi{Agree} with the \isi{φ-features} of T, and many fully
agreeing Ts are possible.

\section{Conclusion and open questions}

In this squib, we investigated the nature of nominative\is{nominative case} and \isi{accusative case} in
Greek. We argued that while accusative qualifies as \isi{dependent case} on the basis
of \citegen{Baker2015} criteria, nominative\is{nominative case} is problematic: while \ili{Greek}
Nominative behaves like unmarked case and is clearly not assigned under
agreement in T in a number of environments, unlike \ili{English}, agreement always
goes with \Nom{} when both are present, like \ili{English}. An important characteristic
of \ili{Greek} not shared by \ili{English} is that it pervasively shows long-distance
chains involving a single in situ \Nom{} subject and many T heads fully agreeing
with it. We suggested that \ili{Greek} has T with interpretable \isi{φ-features} as a
by-product of V \isi{raising} satisfying the \gls{EPP}. This allows for the formation
...
argued that the analysis of \ili{Greek} nominative\is{nominative case} case in connection to agreement
...
\isi{Agree}, see \citet{Schutze2001,McFadden2004}.\footnote{Thanks to Terje Lohndal
