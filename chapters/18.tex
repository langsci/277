...
\ea[*]{[\textsubscript{CP} Who [\textsubscript{TP}
    ...
    recognized \emph{e}\textsubscript{\Pg{}}]]]?}\label{ex:key:18.2}
\z
...
The article is organized as follows: \Cref{sec:key:18.2} introduces the phenomenon
of parasitic agreement\is{agreement!parasitic agreement}. \Cref{sec:key:18.3} discusses semantic and categorial
...
In \Cref{sec:key:18.4}, multiple parasitism is discussed, that is, the appearance
of more than one parasitic agreement\is{agreement!parasitic agreement} morpheme within the adjectival projection.
\Cref{sec:key:18.5} discusses a string-based analysis of parasitic agreement\is{agreement!parasitic agreement}, and
\Cref{sec:key:18.6} discusses a structure-based approach according to which the
intensifier and the gradable adjective are represented as separate attributive
modifiers within the noun phrase.  \Cref{sec:key:18.7} presents the analysis
adopted in this article: parasitic agreement\is{agreement!parasitic agreement} as a manifestation of the
Spec-head agreement configuration. In \Cref{sec:key:18.8} the phenomenon of
parasitic agreement is associated with emphasis of information.
\Cref{sec:key:18.9} concludes the article.
...
As \eqref{ex:key:18.4}--\eqref{ex:key:18.5} show, attributive adjectives in \ili{Dutch}
...
\Cref{sec:key:18.8} for an argument in support of the presence of this
zero-affix.
...
\Cref{sec:key:18.8}.} This is shown by (\ref{ex:key:18.8}a), where we have an
...
inflection on the adjectival head, as is exemplified in \REF{ex:key:18.11}.
...
As indicated by the examples in \REF{ex:key:18.13} modal, temporal or evaluative
modifiers are never augmented with \textsc{-e}.

\ea%13
    \label{ex:key:18.13}\ili{Dutch}
    \sn
...
\eqref{ex:key:18.6}, and also those in \REF{ex:key:18.12}, (categorially) are adverbs
...
\REF{ex:key:18.14}, these intensifying elements occur in syntactic positions that
...
‘rather/fairly’, which are, respectively, an amplifying intensifier\is{intensifiers} and a
downtoning one. As \REF{ex:key:18.15} shows, augmentation with –\textsc{e} is
...
Limburgian \ili{Dutch}. The augmented form \emph{zere} suggests that in this
variety of \ili{Dutch}, \emph{zeer} is adjectival.}

\ea%15
    \label{ex:key:18.15}\ili{Dutch}
    \sn
...
As shown in \REF{ex:key:18.16}, the degree modifier \emph{zeer} does not appear in
positions where adjectives are typically found.

\ea%16
    \label{ex:key:18.16}\ili{Dutch}
    \ea[*]{
...
\REF{ex:key:18.18}, it is impossible to \enquote{skip} a potential carrier of
...
\citealt{Corver1997a,Broekhuis2013}).\footnote{A reviewer points out
    that the restriction on \enquote{spreading} in \REF{ex:key:18.18} is
...
given in \REF{ex:key:18.19}:

\ea%19
    \label{ex:key:18.19}\ili{Dutch}
    \ea[]{
...
Schematically, we have the process as depicted in \REF{ex:key:18.20}, where
...
(\ref{ex:key:18.20}b). This is exemplified in \REF{ex:key:18.21}, where
...
Some relevant facts are given in \REF{ex:key:18.22}:

\ea%22
    \label{ex:key:18.22}\ili{Dutch}
	\ea
...
Importantly, the examples in \REF{ex:key:18.22} show that, in spite of the
...
add augmentative \textsc{-e} to the adjectival degree word\is{degree words}. This suggests that
...
\REF{ex:key:18.23} is completely in line with (\ref{ex:key:18.8}a). Importantly, the
patterns in \REF{ex:key:18.22} and \REF{ex:key:18.23} suggest that what matters for
...
\REF{ex:key:18.25}, where the degree word\is{degree words} modifies a past/passive participle:

\ea%25
    \label{ex:key:18.25}\ili{Dutch}
	\ea
...
The examples in \REF{ex:key:18.24} and \REF{ex:key:18.25} show that
...
So, what underlies the contrast between \REF{ex:key:18.24} versus
(\ref{ex:key:18.26}a), and \REF{ex:key:18.25} versus (\ref{ex:key:18.26}b)?

From a string-based perspective, there is no difference as regards the distance
between the inflected present participle and the modifying degree word\is{degree words}. So
...
Specifically, the categorial nature of the participles in \REF{ex:key:18.24}-(25)
is verbal, while that of the participles in \REF{ex:key:18.26} is adjectival (see
also \citealt{Broekhuis2013} for discussion). The verbal nature of the
participles in \REF{ex:key:18.24}-(25) is clear from their aspectual properties.
The present participles in \REF{ex:key:18.24} express durative aspect, as is clear
...
affairs) of being excited. The past/passive participles in \REF{ex:key:18.25}
...
in \REF{ex:key:18.26} is confirmed by a number of diagnostics for adjectival
status (see also \citealt{Broekhuis2013}). Firstly, synthetic comparative
formation (\emph{-er}) can apply to these forms, as in \REF{ex:key:18.27}.

\ea%27
    \label{ex:key:18.27}\ili{Dutch}
	\ea
...
Thirdly, the participles in \REF{ex:key:18.26} can be modified by the intensifier\is{intensifiers}
\emph{heel} ‘very’ (see \REF{ex:key:18.29}), an intensifier\is{intensifiers} that can combine with
...
\REF{ex:key:18.24}--\REF{ex:key:18.25}. In \REF{ex:key:18.30}, this is exemplified for
\emph{twijfelend} in \REF{ex:key:18.24}:

\ea%30
    \label{ex:key:18.30}\ili{Dutch}
    \ea[*]{
...
(\ref{ex:key:18.31}b), it is an adjectival participle. parasitic agreement\is{agreement!parasitic agreement} is
...
of participles in \REF{ex:key:18.24}, \REF{ex:key:18.25} and (\ref{ex:key:18.31}a), the
...
\Cref{sec:key:18.7}), I propose that the adjectival participle and the verbal
...
contrast between \REF{ex:key:18.24}-(25), on the one hand, and \REF{ex:key:18.26}, on
...
(\ref{ex:key:18.33}b). Being in an attributive position, the adjectival degree word\is{degree words}
...
not work. First of all, as shown in \REF{ex:key:18.34}, \textsc{-e} can also
...
Secondly, patterns like \REF{ex:key:18.35} are possible, in which a PP that is
selected by the adjective precedes the augmented degree word\is{degree words}.

\ea%35
    \label{ex:key:18.35}\ili{Dutch}
    \sn
...
attributive AP, as is depicted in \REF{ex:key:18.36}.

\ea%36
    \label{ex:key:18.36}\ili{Dutch}
    \sn
...
inflection on \ili{Dutch} attributive adjectives: +/- definite, +/- singular,
...
From the examples in \REF{ex:key:18.22} and \REF{ex:key:18.23} we may conclude that
...
\citealt[423]{Broekhuis2013}). As shown in \REF{ex:key:18.17}, \emph{heel} cán
...
phrases containing multiple modifiers. As shown in \REF{ex:key:18.39}, the degree
word is always closest to the gradable adjective:

\ea%39
    \label{ex:key:18.39}\ili{Dutch}
    \ea[]{
...
(attributive) adjective, let us next turn to the pattern in \REF{ex:key:18.18},
repeated here as \REF{ex:key:18.42}:

\ea%42
    \label{ex:key:18.42}\ili{Dutch}
    \ea[]{
...
Before giving an analysis of the (multiple) parasitic agreement\is{agreement!parasitic agreement} phenomenon in
(\ref{ex:key:18.42}c), let me point out that the amplifier \emph{heel} can be
followed only by the amplifying degree word\is{degree words} \emph{erg}. Other \isi{degree words} such
...
\ea%43
    \label{ex:key:18.43}\ili{Dutch}
    \sn[*]{
    \gll een    [heel  vreselijk/ontzettend    dure]              fiets\\
...
\REF{ex:key:18.43}, which express that the size of the interval between the
standard value and the higher degree is \enquote{really big}. It seems that the
amplifier \emph{heel} in \REF{ex:key:18.44} marks the (big) size of the interval
between the standard value and the higher point on the scale.

Let us return to the patterns in \REF{ex:key:18.42} and see how the
(im)possibility of parasitic \textsc{-e} can be accounted for. In
(\ref{ex:key:18.42}a), there is no parasitic agreement\is{agreement!parasitic agreement}. The attributive adjective is
...
\REF{ex:key:18.31}, repeated here as \REF{ex:key:18.46}:

\ea%46
    \label{ex:key:18.46}\ili{Dutch}
	\ea
	\gll een [ hem  erg(*-\textsc{e})  opwindend-e ]    jurk\\
    a   {}     him  very(-\textsc{e})    exciting-\Agr{} {} dress\\
	\ex
...
adjectival part, viz., \emph{-ende}. Since the degree word\is{degree words} does not stand in a
local Spec-head relation with the inflected participial ending \emph{-ende}, it
is not able to display parasitic agreement\is{agreement!parasitic agreement} morphology.
...
From the minimal pair in \REF{ex:key:18.46} and the structure in \REF{ex:key:18.47}
...
adjectives in \REF{ex:key:18.48}. The characterization \enquote*{deverbal} comes
...
As shown in \REF{ex:key:18.48}, the PP-complement can appear either at the left
...
remarkable, since, as was shown in \REF{ex:key:18.40}, the PP-complement cannot
...
asymmetry between the patterns in \REF{ex:key:18.48} and those in \REF{ex:key:18.40}
suggests that the deverbal adjectives in \REF{ex:key:18.48} have, or can have, an
...
\emph{opwindend} in \REF{ex:key:18.47}, the deverbal adjectives in \REF{ex:key:18.48}
...
\Cref{sec:key:18.5}). That is, the inflected attributive adjective and the
adjectival degree word\is{degree words} must be linearly adjacent for inflection to spread onto
the degree word\is{degree words}. As I have argued in \Cref{sec:key:18.5}, however, there are
...
This is exemplified in \REF{ex:key:18.55}:
...
words, one might propose that \emph{-e} in \REF{ex:key:18.55} is licensed by the
...
normally does not license the appearance of \textsc{-e}; see \REF{ex:key:18.57}.
...
\REF{ex:key:18.58}-(59), it cannot be the whole story. Under such an analysis, one
...
The contrast between \REF{ex:key:18.58}-(59), on the one hand, and
\REF{ex:key:18.60}, on the other hand, suggests that some property of the gradable
...
\REF{ex:key:18.58}--\REF{ex:key:18.59} would have the structure in
