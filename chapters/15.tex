...
\ea\label{ex:key:15.1}
    ...
    \z
\z
In (\ref{ex:key:15.1}a) the head of the relative clause\is{relative clauses} has the genitive\is{genitive case} case imposed by the
preposition in the matrix while the relative pronoun\is{relative pronouns} has the accusative
case\is{accusative case} imposed by the embedded verb ‘know’. In (\ref{ex:key:15.1}b) however, the case of the
...
\ea\label{ex:key:15.2}\ili{Greek}
    \ea
    ...
    \z
\z
(\ref{ex:key:15.2}a) is a headed relative clause\is{relative clauses} in which the head is in the dative\is{dative case} case
...
of the verb in the relative clause\is{relative clauses}. (\ref{ex:key:15.2}b) is the corresponding FR\@. As there is
...
attraction which we might call case superimposition. In (\ref{ex:key:15.2}b) the matrix dative\is{dative case}
...
This will  correctly predict that in (\ref{ex:key:15.2}b) it is the dative\is{dative case} that wins and
...
\ea\label{ex:key:15.5}\ili{German}
    \ea[]{%
...
As I will suggest at the end of \Cref{sec:key:15.3}, there is only one
...
overview of the relevant literature. An updated version of this chapter will
appear in \textcite{VanRiemsdijk2017}.} that \glspl{FR} should be treated in
...
\ea[*]{a far from the airport hotel}\label{ex:key:15.9}
\z
...
\ea\label{ex:key:15.11}\ili{German}
    \ea[]{%
...
%\emph{\figref{fig:key:2}:FR analysis by grafting}
...
\ea\label{ex:key:15.16}\ili{Dutch}
    \sn
...
\end{itemize}
\ea\label{ex:key:15.20}\ili{German}
    \ea[]{%
...
\ea\label{ex:key:15.21}\ili{German}
    \ea[*]{%
...
\glspl{FR}.\footnote{(\ref{ex:key:15.22}a) is an example of a case mismatch\is{case!case mismatches} in which the
...
\ea\label{ex:key:15.22}\ili{German}
    \ea[]{%
...
the case morphology. Accordingly the case mismatch\is{case!case mismatches} in (\ref{ex:key:15.22}b) causes
...
\ea\label{ex:key:15.27}\ili{Dutch}
...
\ea\label{ex:key:15.28}\ili{Dutch}
    \sn
    \gll    een wat    ik zou     noemen eenvoudig-*(e) oplossing  \\
            a      what I  would call         simple               solution\\
    \glt
...
\ea\ili{German}
    \sn[*]{%
    \gll    Was viele ein\tss{\Nom{}} geiler\tss{\Nom{}} Wagen nennen\textsuperscript{\Acc{}} wird oft gekauft\textsuperscript{\Nom{}}.\\
...
\ea\label{ex:key:15.31} Case mismatch with \gls{TFR}\\*
    \hspace*{-1cm}\begin{tikzpicture}[baseline]
...
(\ref{ex:key:15.22}a) there is still a conflict, but according to the case
hierarchy \Acc{} supersedes \Nom{}. And indeed, this example is perfect for
some varieties of German and definitely much better than (\ref{ex:key:15.22}b)
for all speakers.\footnote{See also example \eqref{ex:key:15.5} above and
\cref{fn:15.6}.}
...
\ea\label{ex:key:15.32}\ili{German}
    \sn
