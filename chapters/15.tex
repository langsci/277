...
\ea\label{ex:15.12} \gls{FR} analysis by grafting\\*
    \begin{tikzpicture}[baseline]
        \begin{scope}
...
\ea\label{ex:15.24} TFR analysis by grafting\\*
    \begin{tikzpicture}[baseline]
        \begin{scope}
            \node [text width=5cm] (treeA) {input tree A (matrix/host):};
        \end{scope}
...
example, the adjective \emph{simple} with the noun \emph{matter} in \eqref{ex:15.10}.
...
\ea\label{ex:15.26} \enquote*{Attributive} \glspl{TFR}\\*
    \begin{tikzpicture}[baseline]
...
\eqref{ex:15.29}, where the AIs remain outside the shared adjective which is
...
\ea\label{ex:15.29} Mismatch avoidance with attributive adjectives\\*
    \begin{tikzpicture}[baseline]
...
\eqref{ex:15.29} as we have two separate positions.  A conflict is avoided because one of
...
Take example (\ref{ex:15.22}b), repeated here as
\eqref{ex:15.30}.\newpage
...
\z
The structure for such a \gls{TFR} would be roughly as in \eqref{ex:15.31}.

\ea\label{ex:15.31} Case mismatch with \gls{TFR}\\*
...
for all speakers.\footnote{See also example \eqref{ex:15.5} above and
\cref{fn:15.6}.}
...
above, repeated here as \eqref{ex:15.32}. \eqref{ex:15.33} is a very
...
\ea\label{ex:15.33} Case mismatch resolved by superimposition\\*
    \begin{tikzpicture}[baseline]
