...
while revealing new facets of the phenomenon\footnote{For instance also in case
    ...
    overt complementizer\is{complementizers}, but a clear contrast with subject extraction
    persists: \citet{Schippers2012}.}. There are factors of empirical
    complexity, though: certain varieties of \ili{English} admit (\ref{ex:key:21.1}a) as
    ...
    extraction from the weak island\is{islands} is always degraded to some extent):
...
permits a very limited variety of \gls{UG}\is{Universal Grammar} principles: principles operating at the
interfaces with sound and meaning, and somehow enforced by the needs of the
interface systems (e.g., linearization at the \glsunset{PF}\gls{PF} side, the Theta Criterion at
...
labeling,\is{labelling} etc.).
The \gls{ECP}\is{Empty Category Principle} does not naturally fit into any of these categories, so it has no
...
\begin{exe}
    \exi{(5ʹ)} \isi{Criterial freezing}: In a criterial configuration, the criterial
...
attempts to derive the effects of (5ʹ) from the labeling\is{labelling} algorithm. I will not
...
\is{topic} – comment or \isi{focus} – presupposition articulations. So, what could be an
...
with an inherent case\is{case!inherent case} may be a KP, in languages like Italian it could be a KP
or a PP, but in any event it plausibly is an “extended projection” the nominal
element, in \citegen{Grimshaw:1991} sense, hence accessible to being attracted
by a +N attractor.\footnote{That the dative\is{dative case} experiencer is in subject
    position, and not a \is{topic}, is shown, among other things, by the fact that it
    does not interfere at all with Aʹ-bar extraction, whereas a genuine \is{topic} does:
...
\emph{[la} \emph{première} \emph{partie} \emph{[\tss{PP}} \emph{de} \emph{ce}
\emph{roman]]} “the first part of this novel”) and an NP (contained in a larger
structure headed by a numeral, such as \emph{[trois} \emph{[\tss{NP}}
...
\z
%
Here, contrary to \emph{en} extraction in \ili{French}, both A- and Aʹ-movement
...
follows from \isi{criterial freezing} and the subject criterion, which provide an
alternative to the classical \gls{ECP}\is{Empty Category Principle} analysis.
...
Principle} violation: \isi{movement} from the left-peripheral subject position to the
...
interpretation of the structure, e.g., as expressing the \is{topic} – comment or
...
\emph{that} – trace effect, can be made to follow from freezing.
Subject--object asymmetries follow from the fact that there is a subject
