...
do not in fact challenge (\ref{scu}).\footnote{I restrict the discussion here
    ...
    \isi{control} cross-linguistically, \cite{stiebels07} presents Austronesian and
...
\cite{Vergnaud1977,Chomsky1980} proposed that the distribution of \isi{control} is
...
For example, in an ergative--absolutive \isi{alignment}, the transitive subject bears
\isi{ergative case} while the object and intransitive subject share the typically
unmarked \isi{absolutive case}. Case and subjecthood do not align: there is no case
that is assigned to all and only subjects. If Vergnaud were right, and the
account of \isi{control} was that some designated case is unavailable in non-finite
contexts, then the \isi{control} pattern should track case, rather than subjecthood.
For example, an absolutive pattern\is{absolutive alignment} of \isi{control} would look like the following,
with PRO as the object, but not the subject, of a transitive infinitive:

\ea \label{abscntr} 	\nobreak\begin{xlista}
	\ex[]{Leo tried [ PRO$_{\text{ABS}}$  to win. ]}
	\ex[*]{Leo tried [ PRO$_{\text{ERG}}$ to choose his teacher$_{\text{ABS}}$. ]}
	\ex[]{Leo tried [ his teacher$_{\text{ERG}}$ to choose PRO$_{\text{ABS}}$. ]}
	\end{xlista}
\z
...
\cite[319]{polinsky16} illustrate Dixon's observation nicely. \ili{Tsez} has an
...
of language type.\footnote{\cite{Legate2008} defends a version of Case Theory
...
clitic-climbing in \ili{Romance}. In (\ref{clcl}), the object clitic corresponding to
...
\enquote{sharing} is implemented as \isi{control} or \isi{raising} and that unification is
...
\cite{malchukov14} refers to this pair to argue that \isi{control} may, if rarely,
...
of \ili{Itelmen} in (\ref{tree}) in which the apparent ergative--absolutive pattern
is a quirk of agreement morphology and not a matter of the syntax of \isi{control}.
...
familiar from \ili{Latin} and \ili{Greek} \citep{baermanetal07}: syntactically transitive,
but morphologically intransitive. More specifically, their agreement follows an
\isi{absolutive alignment}. Their analysis of the representation of \emph{nus-} with
