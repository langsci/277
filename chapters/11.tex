...
\textsc{Alignment} is the grouping of \isi{grammatical functions} (such as subject,
object; henceforth \glsunset{GF}\glspl{GF}) across transitive and intransitive
...
grouping ergative type. Thus in \ili{German} \eqref{ex:key:11.1}, an accusative language (where case
...
\ea%1
    \label{ex:key:11.1}\ili{German} ({cf.\ \citealt{Curme1952}})\\
	\ea
...
Contrasting with this, in \ili{Coast Tsimshian} \eqref{ex:key:11.2}, an
...
\ea%2
    \label{ex:key:11.2}\ili{Coast Tsimshian} {\parencite[32]{Mulder1994}}\\
	\ea
...
\enquote{Canonical} \isi{ergativity}, as illustrated in \eqref{ex:key:11.2} for
\ili{Coast Tsimshian} case, combines the ergative (\ref{ex:key:11.3}a) and absolutive (\ref{ex:key:11.3}b)
...
in \ili{Indo-Aryan} languages (i.e.\ property (\ref{ex:key:11.3}a)) is not
...
In \ili{German} \eqref{ex:key:11.1} we saw that both subjects and objects are marked for case, along
...
object on the verb in \ili{German}. This is different from, say, \ili{Swahili}, where both
...
agreement:\footnote{See \citet[235--236]{Creissels2000} for a discussion of the
    conditions favoring object agreement\is{agreement!object agreement} marking in \ili{Bantu} languages. This
    touches on the phenomenon of \isi{differential object marking}, which we cannot
    discuss in any detail within the confines of this article. Suffice it to
    say here that \isi{differential object marking} may affect the
    completeness/incompleteness typology in various ways, depending on the
    factor that determines the marking. To take the example of object agreement\is{agreement!object agreement}
    in \ili{Bantu} languages, in some cases, where only topics\is{topic} trigger object
    agreement,\is{agreement!topic agreement}\is{agreement!object agreement} one might argue that object agreement\is{agreement!object agreement} is qualitatively different
    from subject agreement,\is{agreement!subject agreement} and agreement would no longer be complete. On the
    other hand, in cases where object agreement\is{agreement!object agreement} is a function of definiteness
of the object, as in \ili{Swahili}, we may take object marking to involve an
overt/covert opposition, still within the complete type.}\newpage

\ea%5
    \label{ex:key:11.5}\ili{Swahili} \parencite[18]{Barrett-Keach1980}
    \sn
    \gll Juma    a-li-(ki)-soma  ki-tabu\\
        Juma\eqref{ex:key:11.1}  1-\Pst-7-read    7-book\\
...
describe the optional presence of the object agreement\is{agreement!object agreement} marker \emph{ki} in \eqref{ex:key:11.5}
...
\ea%6
    \label{ex:key:11.6}\ili{Spanish} {\parencite[80]{Leonetti2004}}
    \sn
...
\ea%7
    \label{ex:key:11.7}\ili{Paumarí} {\parencite[164]{ChapmanDerbyshire1991}}
    \sn
...
\ea%8
    \label{ex:key:11.8}\ili{Paumarí} {\parencite[163]{ChapmanDerbyshire1991}}
    \sn
...
This would appear to be a tell-tale sign of \isi{ergativity} (property (\ref{ex:key:11.3}a)).
...
\ea%9
    \label{ex:key:11.9}\ili{Paumarí} {\parencite[197]{ChapmanDerbyshire1991}}
    \sn
...
\ea%10
    \label{ex:key:11.10}\ili{Paumarí} {\parencite[197]{ChapmanDerbyshire1991}}
    \sn
...
S\textsuperscript{T}, once more suggesting \isi{ergativity} (see \eqref{ex:key:11.7} vs. \eqref{ex:key:11.8}).
...
\ea%11
    \label{ex:key:11.11} \ili{Paumarí} \parencite[281]{ChapmanDerbyshire1991}
    \sn
...
\ea%12
	...
	\ea S\textsuperscript{T} = S\textsuperscript{I} = O \tab{\textbf{identical}}
	\ex S\textsuperscript{T} = S\textsuperscript{I} ≠ O \tab{\textbf{accusative}}
	\ex S\textsuperscript{T} ≠ S\textsuperscript{I} = O \tab{\textbf{ergative}}
	\ex S\textsuperscript{T} = O ≠ S\textsuperscript{I} \tab{\textbf{intransitive}}
	\ex S\textsuperscript{T} ≠ S\textsuperscript{I} ≠ O \tab{\textbf{tripartite}}
	\z
\z
...
\ea%13
    ...
    \ea only S\textsuperscript{T}/S\textsuperscript{I}
        \tab{\makebox[0pt][r]{i.}\quad S\textsuperscript{T} = S\textsuperscript{I}} \tab{\textbf{subjective}}\\
        \tab{\makebox[0pt][r]{ii.}\quad S\textsuperscript{T} $>$ S\textsuperscript{I}} \tab{\textbf{transitive subjective}}\\
        \tab{\makebox[0pt][r]{iii.}\quad S\textsuperscript{T} $<$ S\textsuperscript{I}} \tab{\textbf{intransitive subjective}}
	\ex only S\textsuperscript{I}/O
        \tab{\makebox[0pt][r]{i.}\quad S\textsuperscript{I} = O} \tab{\textbf{absolutive}}\\
        \tab{\makebox[0pt][r]{ii.}\quad S\textsuperscript{I} $>$ O} \tab{\textbf{intransitive absolutive}}\\
        \tab{\makebox[0pt][r]{iii.}\quad S\textsuperscript{I} $<$ O} \tab{\textbf{transitive absolutive}}
	\ex only S\textsuperscript{T}/O
        \tab{\makebox[0pt][r]{i.}\quad S\textsuperscript{T} = O} \tab{\textbf{transitive}}\\
        \tab{\makebox[0pt][r]{ii.}\quad S\textsuperscript{T} $>$ O} \tab{\textbf{subjective transitive}}\\
        \tab{\makebox[0pt][r]{ii.}\quad S\textsuperscript{T} $<$ O} \tab{\textbf{objective transitive}}
    \ex only O                    \tab{} \tab{\textbf{objective}}
    \ex only S\textsuperscript{T} \tab{} \tab{\textbf{narrow ergative}}
    \ex only S\textsuperscript{I} \tab{} \tab{\textbf{narrow intransitive}}
	\z
\z
...
and in fact tripartite ((\ref{ex:key:11.12}e), e.g.\ \ili{Paumarí} for case),
but if not, the language is incomplete for γ, and in fact subjective
((\ref{ex:key:11.13}a), e.g.\ \ili{Paumarí} for agreement).
...
(cf.\ \eqref{ex:key:11.14}), we may identify \ili{German} as an accusative
language.
...
In \ili{Wambaya} (\citealt{Nordlinger1998}; cf.\ \cref{fn:5}), case is marked on
...
where S\textsuperscript{T} is marked by \emph{-(n)im}, O by \emph{-ne}, and
...
\section{Some consequences}\label{sec:key:11.5}
...
saw this illustrated for case in \ili{Coast Tsimshian} \eqref{ex:key:11.2}. \ili{Wambaya} is also ergative
...
\ea%15
    \label{ex:key:11.15}\ili{Malimiut Iñupiaq} {\parencite[188]{Lanz2010}}\\
	\ea
		\gll iγlaq-tu-ŋa\\
			laugh-\Intr.\Ind-\Fsg{}\\
		\glt ‘I am laughing.’
...
narrow ergative type (\ref{ex:key:11.13}e). We saw this for case in \ili{Paumarí} unmarked word
...
case in \ili{Coast Tsimshian} \eqref{ex:key:11.2} and also in \ili{Wambaya}
(except for pronouns). For agreement the complete ergative type is illustrated
in \ili{Malimiut Iñupiaq} \eqref{ex:key:11.15}.
...
\ea%16
    \label{ex:key:11.16} \ili{Tsez} \parencite[345]{Polinsky2014}\\
	\ea
		\gll isi        y-ay-s \\
...
(see \eqref{ex:key:11.14}), the family of \isi{alignment} types that treat all
...
\ea%17
\label{ex:key:11.17} \ili{Kham} \parencite[66--67]{Watters2002}\\
	\ea
...
marked by a special \isi{accusative case} in (\ref{ex:key:11.17}c), yielding what looks like an
accusative pattern. The ergative and accusative patterns can also be mixed:

\ea%18
    \label{ex:key:11.18} \ili{Kham}\\
	\ea
...
\Cref{sec:key:11.4}), but complete for agreement.
...
\citep{Polinsky2014}.\footnote{Another case could be \ili{Marathi} (and similar
    ...
    text (\Cref{sec:key:11.4}).  Also, the absolutive-looking agreement pattern shows up in all past
...
\ea%19
    \label{ex:key:11.19}\ili{West Greenlandic} {\parencite[55]{Bittner1994}}\\
    \ea[]{
...
\ea%20
    \label{ex:key:11.20} Tongan \parencite[81]{Otsuka2006}\\
    \ea[]{
...
In both \ili{West Greenlandic} \eqref{ex:key:11.19} and \ili{Tongan}
\eqref{ex:key:11.20}, straightforward relativization of S\textsuperscript{T} is
...
\ea%21
    \label{ex:key:11.21}\ili{West Greenlandic} {\parencite[58]{Bittner1994}}
    \sn
...
\ea%22
    \label{ex:key:11.22}\ili{Tongan} {\parencite[81]{Otsuka2006}}
    \sn
...
Aʹ-moved elements along ergative lines include \ili{Abaza}, \ili{Selayarese},
...
different \isi{alignment} types, both within the ergative family \eqref{ex:key:11.14}, namely
...
We noted in \Cref{sec:key:11.5.4} that no correspondence between case and
...
function of \isi{Merge}.\footnote{A related question is whether \isi{ergative case} should
    ...
    \Cref{sec:key:11.5.1}), it is unlikely that this question can be given a
