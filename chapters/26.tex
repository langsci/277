...
change was further aggravated by the entry of \emph{a gente} (the folks, the
people, similar in meaning to \ili{French} \enquote*{on}), in Paradigm 3,
replacing first person plural \emph{nós} (we), also requiring the unmarked
third person singular agreement, due to its nominal origin.
...
\parencite{Duarte1995}.\footnote{In short answers we can have an apparent NS
    ...
    identified (\ref{ex:key:26.6}a); first person singular null subjects reach
    25\%, particularly when preceded by a functional category, such as a NegP,
    and AspP (\ref{ex:key:26.6}b):
...
\gls{BP}\il{Brazilian Portuguese} (cf.\ the examples in \eqref{ex:key:26.18},
...
\ea%44
    ...
    \ex cf.\ English\\
    \textbf{Você} / \textbf{a gente} nao pode entrar de sapato.\\
        ‘You / {the folks} can’t get in with your / one’s shoes on.’
    \z
\z
...
%                    nodes near coords align={horizontal},
        ...
        \foreach \i in {1,...,\numberofcols} {%
                \addplot+ table [x index={1},y index={\i},x expr=\thisrow{Type}] {\table};
                \pgfplotstablegetcolumnnamebyindex{\i}\of{\table}\to{\colname} % Adding column headers to legend
            %\addlegendentryexpanded{\colname}
        }
        ...
        \node [RdYlBu-F!60!black, above] at (axis cs: 1, 58) {58};
        \node [RdYlBu-F!60!black, above] at (axis cs: 2, 85) {85};
        \node [RdYlBu-F!60!black, above, yshift=-2] at (axis cs: 3, 93) {93};
        \node [RdYlBu-D, above] at (axis cs: 1, 21) {21};
        \node [RdYlBu-D, above] at (axis cs: 2, 10) {10};
        \node [RdYlBu-D, above] at (axis cs: 3, 6) {6};
        \node [RdYlBu-A, below] at (axis cs: 1, 21) {21};
        \node [RdYlBu-A, left, yshift=-5] at (axis cs: 2, 5) {5};
        \node [RdYlBu-A, right, yshift=3] at (axis cs: 3, 1) {1};
        \end{axis}
    \end{tikzpicture}
...
\emph{semi} \emph{NS} language, the expletive\is{expletives} can be overt or
...
\ea%50
\label{ex:key:26.50}\ili{Icelandic} \parencite[160]{SigurdssonEgerland2009}
    \ea
