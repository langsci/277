...
\begin{figure}
    \caption{{\color{red}Please insert caption}\label{fig:ex:26.1}}
...
\begin{figure}
    \caption{{\color{red}Please insert caption}\label{fig:ex:26.2}}
...
type of language”  (cf.\ \ref{ex:26.3}a) and also a \gls{NSL}\is{null subject languages} (cf.\
\ref{ex:26.4}a) (\citealt{Adams1987}, \citealt{Roberts1993}, a.o.). The
...
language”\footnote{Cf.\ \textcite{Ribeiro1995} for OP and
    ...
    Portuguese as a V2\is{V2 word order} language.}  (cf.\ \ref{ex:26.3}b).
...
\begin{figure}[p]
    ...
    \begin{tikzpicture}
        \begin{axis}[
                    ybar,
                    nodes near coords,
                    cycle list = {{black!90, fill=black}, {black!40, fill=black!50}},
                    height = 4cm,
                    legend cell align=left,
                    legend columns=3,
                    legend style={font=\footnotesize,anchor=north,at={(0.5,1.2)},anchor=north},
...
\eqref{ex:26.25} below as a matter of acquisition\is{language acquisition}, in line with what we saw
in \Cref{sec:26.2.3}.  According to these authors, children start with the
parameter in \gls{BP}\il{Brazilian Portuguese} set as [$-$]NS, with only
(\ref{ex:26.25}a) as a possibility, but later, in view of the input of
