...
\ea\label{ex:key:17.16}
    \eqref{ex:key:17.6} = [\tss{\#P} [\tss{DP} \dots{} D\tss{\#} \dots{} ] [\tss{IP} I\tss{\#} \dots{} ]]
\z
...
\ea\label{ex:key:17.18}
    \ea[*]{(\ref{ex:key:17.1}b) = [\tss{\{}\tss{π}\tss{\#\}P} [\tss{DP} \dots{} D\tss{\#} \dots{} ] [\tss{IP} I\tss{\{}\tss{π}\tss{,\#\}} \dots{} ]]}
...
algorithm assumed in~\Cref{sec:key:17.2} then requires that \# is higher than
...
case of “\isi{unagreement}” in the Spanish~\eqref{ex:key:17.6} by proposing
...
to~(\ref{ex:key:17.39}a) as a representation of the \ili{Spanish}
(\ref{ex:key:17.40}):
...
the π. Since \ili{Spanish} allows this,~\eqref{ex:key:17.43} is fine as far as
...
labelling\is{labelling} largely predicts the effects of~\eqref{ex:key:17.2}
makes this unlikely.
...
end of \Cref{sec:key:17.6}, but it is not unlikely that there are better ways
