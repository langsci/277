...
In \Cref{sec:key:22.2}, I summarise the reasons why the arguments proposed so
...
\Cref{sec:key:22.3} I outline the argument from \ili{Greek} gerundival clauses
and draw a distinction between manner and absolute/temporal gerunds, of which
only the latter really involve syntactic control. In \Cref{sec:key:22.4} I
...
\Cref{sec:key:22.5}, I conclude and present some implications and
...
predicates \REF{ex:key:22.1}, (b) passives\is{passive}, but not
unaccusatives\is{unaccusativity} or middles,
license non-finite purpose clauses in which PRO is controlled by the IA
\REF{ex:key:22.2}, (c) the \gls{IAg}\is{implicit arguments} can be the
antecedent of reflexive pronouns (with arbitrary reference) \REF{ex:key:22.3},
and (d) internal arguments in passives\is{passive} cannot be coreferential with the
implicit external argument \REF{ex:key:22.4}, a restriction which can be
...
\ea\label{ex:key:22.5}
    \ea \textcite[3]{Landau2010}, adapted from
    \textcite[120--121]{Chomsky1986}\\
    \sn The room was left (*angry).
    \ex[*]{The issue was decided unassisted.}
    \ex[*]{The game was played shoeless.}
    \z
\z

Similarly, \citet{Williams1985} dismisses \REF{ex:key:22.1} on the grounds that
...
The apparent \isi{binding} effects illustrated in \eqref{ex:key:22.3}
...
\citet[219]{AlexiadouEtAl2015}, examples such as \REF{ex:key:22.3} could “find
...
\REF{ex:key:22.4}, this could be made to simply follow directly from the
...
Voice heads attested cross-linguistically \REF{ex:key:22.6}.
...
\citet{Williams1985} proposed that in examples such as \REF{ex:key:22.2} it is
...
then control fails \REF{ex:key:22.7}; likewise, similar S-control phenomena can
be obtained even with unaccusative\is{unaccusativity} predicates, given appropriate additional
context \REF{ex:key:22.8}, or even with events disallowing the participation of
an agent \REF{ex:key:22.9}.

\ea[*]{The boat was sunk [ PRO to become a hero ].}\label{ex:key:22.7}
\z
...
(\ref{ex:key:22.11}a), as per \citegen{Visser1973} generalization, while the
...
generalisation, suggesting that implicit control\is{control!implicit control} is only licit if no overt DP
establishes an \isi{Agree} relation with T, assuming that the expletive\is{expletives} in impersonal
passives does not enter such a relationship. Such a proposal is indeed akin to
the idea pursued in this paper that the \gls{IAg}\is{implicit arguments}, if realised syntactically, should
...
semantic, if \enquote{semantic} is to be understood as \enquote{possible in the
    ...
    has to come from the semantics of the Voice head, as in (6ʹ) below
...
\REF{ex:key:22.12}, the null subject of the gerund can be co-indexed with either
the null subject of the matrix clause or the (cliticised) object. In
\REF{ex:key:22.13}, it is co-indexed with the indirect object of the matrix, and
in \REF{ex:key:22.14} it is shown that it can be co-indexed with object
...
\ea%14
    \label{ex:key:22.14} \ili{Greek} (adapted from \citealt{Anagnostopoulou1999})
    \sn
...
gerunds from taking \glspl{IAg}\is{implicit arguments} as their antecedents. In \REF{ex:key:22.15}, the subject
...
\ea%15
    \label{ex:key:22.15} \ili{Greek} (adapted from \citealt{Kotzoglou2016})
    \sn
...
noteworthy exception \REF{ex:key:22.16}.

\ea%16
    \label{ex:key:22.16} \ili{Greek}
    \sn
...
\isi{passive} [\eqref{ex:key:22.18}, \eqref{ex:key:22.19}], ergative
[\eqref{ex:key:22.20}], or psych predicate [\eqref{ex:key:22.15}]”.

\ea%17
\label{ex:key:22.17} \ili{Greek}
    \sn
...
Crucially, with the exception of the implicit experiencer in \REF{ex:key:22.15},
...
\ea%21
    \label{ex:key:22.21} \ili{Greek}
    \sn[?]{
    \gll    Proigumenos, (vjenondas\tss{j+m} apo   to   ksenodochio) o Janis\tss{j} tis\tss{m}   kratise  (tis Marias\tss{m})\tss{} tin  porta (vjenondas\tss{j+m} apo   to   ksenodochio).\\
...
non-subjects \REF{ex:key:22.25}.

\ea%25
    \label{ex:key:22.25} \ili{Greek}
    \sn
...
\REF{ex:key:22.26}, there must be complete and not partial overlap between the
...
the properties of \citegen{Cinque:1988} \enquote{\emph{quasi-existential}}
...
\REF{ex:key:22.16} above, such understood agents fail to control into absolute
...
\REF{ex:key:22.30}, the internal argument of an unaccusative\is{unaccusativity} predicate is not a
licit controller.

\ea\label{ex:key:22.30} \ili{Greek}
    \sn
...
rather than episodic \REF{ex:key:22.31}.

\ea%31
    \label{ex:key:22.31} \ili{Greek}
    \sn
...
controller in \REF{ex:key:22.31} is an arbitrary, non-referential element, and
...
these properties are manifested in \REF{ex:key:22.31}. \citet{Roberts2014b}
...
That \REF{ex:key:22.31} is no exception to Roberts’ licensing principle is
...
DP/\emph{n}P is a phase\is{phases}). In \REF{ex:key:22.33}, PRO cannot be interpreted as
...
possible verbal configurations in \REF{ex:key:22.32} will have to arise within
...
controllers, episodic sentences like \REF{ex:key:22.36} below the following, are
also possible.

\ea%36
    \label{ex:key:22.36} \ili{Greek}
    \sn
...
people \emph{including} \emph{the} \emph{speaker}.   Even \REF{ex:key:22.16}
paraphrased below as \REF{ex:key:22.37} can have a similar reading for some
speakers, if actually uttered by the policeman who shot the suspect or someone
who was with him:

\ea%37
\label{ex:key:22.37}\ili{Greek}
    \sn[\%]{
    \gll    O    ipoptos   pirovolithike   [ PRO pijenondas   na   ton silavume ]\\
...
\ea%38
    \label{ex:key:22.38}\ili{Italian}
...
typology illustrated in \tabref{tab:key:22.1}.
...
site of the moved head noun \REF{ex:key:22.43}.
...
\parencite{Starke2001,Rizzi2001,Rizzi2013}, summarised in \REF{ex:key:22.44}
...
F\tss{\Gen{}}\textsuperscript{0} \REF{ex:key:22.45}. In other words, the
...
(\ref{ex:key:22.46}a)\footnote{Recall that, unlike other \isi{null subject languages} (e.g.\ \ili{Italian}/\ili{Spanish}), Modern \ili{Greek} lacks participial
...
other hand must have a reduced/defective feature makeup (\ref{ex:key:22.46}b).
...
(\ref{ex:key:22.47}a), as opposed to non-generic \glspl{IAg}\is{implicit
arguments} which are compatible with any [Person] value (\ref{ex:key:22.47}b).
...
from probing and matching the internal argument.

\ea%46
    \label{ex:key:22.46}\leavevmode\\[-1\baselineskip]
    \begin{tabular}{lllll}
...
through. \Cref{tab:key:22.2} presents all the conceivable combinations of the
...
adverbials \REF{ex:key:22.48} -{}- which is therefore not to be taken as a safe
diagnostic for syntactically realised agents).

\ea%48
    \label{ex:key:22.48}\ili{Greek}
    \sn
...
Voice head in \ili{Greek} is the one proposed in \REF{ex:key:22.6} above, without the
presupposition that derives disjointness – repeated (and adapted) here as (6ʹ).
(49a,b) illustrates the relevant contrast between \ili{English} and \ili{Greek}. It remains
to be seen if natural languages do this more widely, i.e.\ whether in the
absence of a syntactic mechanism that can be used for the
grammaticalisation,\is{grammaticalization}
i.e.\ the obligatory expression, of a meaning, related constructions are
employed and the otherwise grammaticalised\is{grammaticalization} meaning is only an inferred meaning.

\begin{exe}
    \exi{(6ʹ)}
    ⟦Middle⟧ = λf\tss{es,t} λe${\exists}$x. f(x)(e)\label{ex:key:22.6prime}
\end{exe}
...
Agree with T is blocked by an intervening \gls{IAg}\is{implicit arguments} which controls into a non-finite
complement clause. Such an explanation would have wider implications for the
analysis of goal passives\is{passive} more generally, but I will leave this issue open for
...
minimality effects, depending on the person feature\is{person features} of the promoted/agreeing
