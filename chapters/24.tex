...
the \isi{control} context in (\ref{ex:key:24.1}b), however, where the interpretation of the embedded
...
\ea\label{ex:key:24.2} \ili{Russian} \parencite[909]{Landau2008}
    \sn
...
\ea\label{ex:key:24.3} \ili{European Portuguese} \parencite[34]{Sheehan2018b}\\
    \sn
...
\ea\label{ex:key:24.4} \ili{Icelandic} \parencite[149]{Sheehan2018b}\\
    \sn
...
\ea\label{ex:key:24.5} \ili{German} \parencite[45]{Landau2000}\\
    \sn
...
\ea\label{ex:key:24.6} \ili{French} \parencite[85]{Landau2000}\\
    \sn
...
\ea\label{ex:key:24.7} \ili{Italian}, adapted from \parencite[46]{Landau2000}\\
    \sn
...
control|see{control}}\index{control!exhaustive control}
...
structure of the squib is as follows. \Cref{sec:key:24.2} reviews the treatment
of finite \isi{control} in previous analyses, notably
\textcite{Landau2004,Landau2015}. \Cref{sec:key:24.3} reviews the evidence for
\gls{PC}\is{control!partial control} in Greek. \Cref{sec:key:24.4} identifies
similar such cases in \ili{Romanian}. \Cref{sec:key:24.5} concludes by discussing the
...
\citealt{Spyropoulos2007b} to whom we will return shortly, this claim is echoed
...
\ea\label{ex:key:24.10} The obligatory control--no \isi{control} generalization\\
...
\ea\label{ex:key:24.11} C-subjunctive, \ili{Greek}\\
    \sn
...
\ea\label{ex:key:24.12} F-subjunctive, \ili{Greek} \parencite[(21)]{Varlokosta1994}\\
...
\ea\label{ex:key:24.13} C-subjunctive, \ili{Romanian} \parencite[6]{Alboiu2007}\\
...
\ea\label{ex:key:24.14} F-subjunctive, \ili{Romanian}\\
...
to require C-subjunctives whereas \gls{PC}\is{control!partial control}
...
\eqref{ex:key:24.15}:\glsunset{OC}
...
Landau proposes to derive \eqref{ex:key:24.15} from the fact that variable \isi{binding} requires
...
\ea%17
    \label{ex:key:24.17} \ili{Greek} \parencite[(34a), ((35a)]{Spyropoulos2007b}
    \sn
...
Romanian, cf.\ \Cref{sec:key:24.4}), suggesting that \eqref{ex:key:24.18} is a
genuine instance of PC:

\ea%19
    \label{ex:key:24.19} \ili{Greek}
    \sn[*]{
	\gll Mporo   na   fas\\
...
that \eqref{ex:key:24.18} is not an instance of \gls{PC}\is{control!partial control} after all.
...
on \ili{Romanian}) (see examples \eqref{ex:key:24.13} and \eqref{ex:key:24.14}
...
\ea%23
    \label{ex:key:24.23}\ili{Romanian}
	\ea
...
\ea%24
    \label{ex:key:24.24}\ili{Romanian}
    \sn[*]{
	\gll Victor încearcă     [ Mihai     să   cânte ]\\
...
\ea%25
    \label{ex:key:24.25}\ili{Romanian} \citep[8]{Alboiu2007}
	\ea
...
\ea%26
    \label{ex:key:24.26} \ili{Romanian}
    \ea[*]{
...
\ea%27
\label{ex:key:24.27}\ili{Romanian}
    \sn[*]{
	\gll Eu vreau   [ să   plec      împreună ]\\
        I   want.\Fsg{} {} \Sbjv{}   leave.\Sbjv{}.\Fsg{}  together\\
    }
\z

\ea%28
\label{ex:key:24.28}\ili{Romanian}
    \sn[*]{
	\gll Vreau     [ să   plecăm       eu  împreună ]\\
...
\ea%29
\label{ex:key:24.29}\ili{Romanian}  \citep[10]{Alboiu2007}
    \sn
...
tell whether \eqref{ex:key:24.29} involves partial \isi{control} or accidental co-reference as
\emph{a vrea} ‘to want’ (like other desiderative predicates) takes an
F-subjunctive which does not force obligatory coreference:

\ea%30
    \label{ex:key:24.30}\ili{Romanian}  \citep[11]{Alboiu2007}
    \sn
...
\ea%31
    \label{ex:key:24.31} \ili{Romanian}
    \sn[*]{
	\gll Tu   poți   să   meargă   mâine.\\
    you   can.\Ssg{}   \Sbjv{}   go.\Sbjv{}.\Third{} tomorrow\\
    \glt}
\z

\ea%32
    \label{ex:key:24.32} \ili{Romanian}
    \sn
...
\ea%33
    \label{ex:key:24.33} \ili{Romanian}
    \sn
...
\ea%34
    \label{ex:key:24.34} \ili{Romanian}
    \sn
...
\ea%35
    \label{ex:key:24.35} \ili{Romanian} \citep[136]{PanaDindelegan2013}
    \sn
	\gll El poate alerga\\
		he can run.\Inf{}\\
	\glt ‘He can run’
...
\ea%36
    \label{ex:key:24.36}\ili{Romanian}
    \sn
...
\ea%37
    \label{ex:key:24.37}\ili{Romanian}
    \sn
...
\ea%38
    \label{ex:key:24.38}\ili{Romanian} (\citealt[194]{PanaDindelegan2013}, citing \citealt[60]{Jordan2009})
    \sn
...
\ea%39
    \label{ex:key:24.39}\ili{Romanian}
    \sn[*]{
	\gll Tu   poți   a vă     căsători   la anul.\\
...
\ea\label{ex:key:24.41} Modal-meet construction schema \parencite[15]{Poole2015}\\%41
...
\ea%42
    \label{ex:key:24.42}\ili{Romanian}
    \sn[*]{
	\gll Alex     se   întâlnește\\
		Alex     \textsc{se}   meet.\Tsg{}\\
    }
\z

\ea%43
    \label{ex:key:24.43}\ili{Romanian}
    \sn
...
\ea%44
    \label{ex:key:24.44}\ili{Romanian}
    \sn
...
\ea%45
    \label{ex:key:24.45}\ili{Romanian}
    \sn
...
\ea%46
    \label{ex:key:24.46}\ili{Romanian}
    \sn
...
\ea%47
    \label{ex:key:24.47}\ili{Romanian}
    \sn
...
\ea%48
    \label{ex:key:24.48}\ili{Romanian}
    \sn
...
\ea%49
    \label{ex:key:24.49}\ili{Romanian}
    \sn
...
\ea%50
    \label{ex:key:24.50}\ili{Romanian}
    \sn
...
\ea%51
    \label{ex:key:24.51}\ili{Greek}
    \sn[*]{
	\gll O Yianis sinantithike\\
		the John  met.\Tsg{}\\
    \glt}
\z

\ea%52
    \label{ex:key:24.52}\ili{Greek}
    \sn
