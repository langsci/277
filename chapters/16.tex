...
\ea\label{ex:key:16.3}\ili{Swedish}\\
    \sn[*]{
    \gll    era                barn       åsikter\\
...
\ea\label{ex:key:16.4}\ili{Finnish}\\
...
\ea\label{ex:key:16.5}\ili{Finnish}
	\ea
...
agreement suffix is ill formed in \ili{Finnish} (\ref{ex:key:16.5}b): they
...
\gls{APC}\is{adnominal pronoun construction} would be (\ref{ex:key:16.7}a),
...
As can be seen in (6a,b), the pronoun in the \gls{APC}\is{adnominal pronoun construction} overtly shows the case
assigned to the DP; nominative\is{nominative case} in (\ref{ex:key:16.6}a), accusative in
(\ref{ex:key:16.6}b).\footnote{The following is an expression in a Facebook message written by
    a native \ili{English} speaker:  (This was) “a good plug for we skipraiders”.
This would be a case where the \isi{accusative case} assigned by the preposition does
not trickle down to the  head of the \gls{APC}\is{adnominal pronoun construction}.\label{fn:16.2}} In \ili{English} the
nominative--accusative distinction is visible only on pronouns. \ili{English} also
has a Genitive\is{genitive case} or possessive case visible on pronouns, as in \emph{my book, our
...
preferable to (\ref{ex:key:16.8}a), either with nominative\is{nominative case} or default
...
(\ref{ex:key:16.8}c), with a left-dislocated \gls{APC}\is{adnominal pronoun construction} combined with a possessor pronoun.

The same holds true of \ili{Swedish}. (9a,b) shows that \ili{Swedish} has the \gls{APC}\is{adnominal pronoun construction}, with
case visible on the pronoun.

\ea\label{ex:key:16.9}\ili{Swedish}
	\ea
...
Standard \ili{Swedish} has the possessive construction in (\ref{ex:key:16.7}b) with
...
\begin{exe}
    \exi{(i)}\ili{Swedish}
    \sn[*]{
	\gll Dom skrattar åt vi/oss barn våra åsikter.\\
...
\ea
    \label{ex:key:16.11}\ili{Finnish}
	\ea
...
(\ref{ex:key:16.11}b) is partitive, one of the object cases in \ili{Finnish}. The
...
(13) shows that the \gls{APC}\is{adnominal pronoun construction} can be a possessor, with Genitive\is{genitive case} marked on both
...
drawing my attention to this interesting and intriguing fact). The \gls{APC}\is{adnominal pronoun construction}
...
In colloquial \ili{Finnish} \eqref{ex:key:16.13} can alternatively mean ‘our children’s opinions are
...
\ea
    \label{ex:key:16.14} \ili{Finnish}
    \sn
...
Standard \ili{Finnish} (\ref{ex:key:16.12}a), (\ref{ex:key:16.12}c) and \eqref{ex:key:16.13}: The \gls{APC}\is{adnominal pronoun construction} possessor does not
...
\ea\label{ex:key:16.16}\ili{Hungarian}
	\ea
...
\ea\label{ex:key:16.17}\ili{Hungarian}
    \ea[*]{
...
nominative\is{nominative case} ungrammatical option in (\ref{ex:key:16.17}a) is a no-case
option, this fact falls under the generalization \eqref{ex:key:16.18}. This idea will be
developed in \Cref{sec:key:16.3}.\footnote{In \ili{Icelandic}, too, the possessor DP
...
therefore quite clearly preferable to (\ref{ex:key:16.19}b) in \ili{Hungarian}.  In \ili{Finnish} there is
...
complex in \ili{Swedish}, possessor pronouns cannot. Consider, for example, \eqref{ex:key:16.21}
...
\ea Swedish\\\label{ex:key:16.25}
    \sn[*]{
    \gll {}[ våra barn ] åsikter\\
        {} our  children {} opinions\\
    \glt Intended: `we childrens opinions'}
\z

\ea[*]{our children opinions}\label{ex:key:16.26}
\z
...
\ea\label{ex:key:16.30}\ili{Swedish}
	\ea
		\gll mina och dina vänner\\
...
the \ili{Swedish} coordinated pronouns in \eqref{ex:key:16.30} are coordinated PossPs
...
\ea\label{ex:key:16.32}\ili{Swedish}
    \sn
	\gll mina vänner och dina\\
		my friends    and yours\\
\z

Many speakers (although not all) agree that the \ili{English} coordinations in \eqref{ex:key:16.33}
...
\eqref{ex:key:16.35}, a \isi{coordination} of two PossPs.
...
valued \isi{φ-features}. In the case of \eqref{ex:key:16.36}, it will find the φ-feature set [\Fpl{}]
...
Consider \ili{Finnish}. The counterpart of \emph{our home} is \eqref{ex:key:16.38}:

\ea\label{ex:key:16.38} \ili{Finnish}
    \sn
    \gll (meidän) koti-mme\\
...
result would look like (\ref{ex:key:16.41}a,b), following \gls{EPP}-driven
movement of the possessor argument to the spec of PossP. The structure of
(\ref{ex:key:16.41}b) would be (\ref{ex:key:16.41}c).\footnote{The well-formed
...
\ea\label{ex:key:16.41}
    \ea[*]{the girl her car}
    \ex[*]{we children our home}
    \ex[]{[\textsubscript{PossP} [we children]
            [\textsubscript{Possʹ} our [\textsubscript{NP} \tuple{we children}
                [\textsubscript{Nʹ} home]]]]}
	\z
\z
...
\ea\label{ex:key:16.42}\textcite[ex. 5]{Allen2002}\\
...
visible when comparing (\ref{ex:key:16.43}b) and \eqref{ex:key:16.44} (cf.
(\ref{ex:key:16.2}a)).

\ea[*]{our children home}\label{ex:key:16.44}
\z
...
discussed in \Cref{sec:key:16.2}, \ili{Swedish} patterns like \ili{English} in this
...
In \Cref{sec:key:16.2} we saw that \ili{Hungarian} shows essentially the same pattern
as \ili{Finnish}, particularly in the case where the possessor has dative\is{dative case} case. Like
Finnish, \ili{Hungarian} has possessor agreement, spelled out as a suffix on the
possessee noun, when the possessor is a pronoun, not when it is a lexical DP.

\ea\label{ex:key:16.46}\ili{Hungarian}
   	\ea
		\gll nektek    a    vélemény-e-tek\\
		        you.\Dat{} the opinion-\Poss-\Spl{}\\
	    \glt `your opinion'
...
As in \ili{Finnish}, the possessor can be an \gls{APC}\is{adnominal pronoun construction}, but only when it has dative\is{dative case} case.
On the assumption that nominative\is{nominative case} case on the possessor,
which is the other alternative in the \ili{Hungarian} possessive construction, means
no case,  the \ili{Hungarian} \gls{APC}\is{adnominal pronoun construction} conforms with the generalization
...
And just as in \ili{Finnish}, when the possessor is an \gls{APC}\is{adnominal pronoun construction}, there is no possessor
agreement. The \gls{APC}\is{adnominal pronoun construction} behaves like a lexical DP in spite of being headed by
a pronoun.\newpage

\ea\label{ex:key:16.48}\ili{Hungarian}
    \sn[?]{
...
relation is Agree, valuation of unvalued \isi{φ-features}. As in certain other cases
