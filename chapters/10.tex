...
The paper is organized as follows. In~\Cref{sec:key:10.2}, I introduce deponent
and deponent-like verbs, that is, the basic patterns that motivate the present
inquiry. \Cref{sec:key:10.3} gives a bird’s eye view of the most common
assumptions on the syntax of voice morphology in the current literature. In
\Cref{sec:key:10.4}, building on my previous work, I present an alternative
...
vs.\ (\ref{ex:key:10.1}b) from \ili{Albanian} – \isi{deponent verbs} familiar first and foremost
...
\ea%1
\label{ex:key:10.1} \ili{Albanian}
    \ea
	\gll    Po    krihem.\\
            \Prog{}  comb.\Fsg.\Nact{}\\
...
(\ref{ex:key:10.2}b) for \ili{Latin}, which as \citet{Grestenberger2018a} notes, can
...
passivize. I postpone the discussion of these verbs to \Cref{sec:key:10.4}.}
The \ili{Albanian} examples in \eqref{ex:key:10.3} further illustrate the point
that deponents do not have formally (i.e. morphologically) active counterparts
(compare with \eqref{ex:key:10.1}).

\NumTabs{5}
\ea\label{ex:key:10.2} \ili{Latin}\\%
...
\begin{exe}\label{ex:key:10.3}
    \ex \ili{Albanian}\\[.25\baselineskip]
...
\eqref{ex:key:10.4}.\footnote{Not all \isi{deponent verbs} in \ili{Latin} are transitive either,
but crucially, unlike in \ili{Albanian}, some are. See also~\Cref{sec:key:10.4}.}

\ea%4
    \label{ex:key:10.4} \ili{Albanian}
    \ea[]{
...
\ea\label{ex:key:10.6} \ili{Italian}
    \ea[]{
...
\ea\label{ex:key:10.7} \ili{German}
    \ea
...
\ea\label{ex:key:10.8} \ili{German}
    \ea[]{
	\gll    Ich  schäme  mich.\\
            I    shame  me/myself\\
...
\ea\label{ex:key:10.11} \ili{Italian}
    \sn
...
\ea\label{ex:key:10.12} \ili{French}
    \sn
...
analysis I provide in~\Cref{sec:key:10.4}.
...
\ea\label{ex:key:10.16} Deponent\\
...
The \ili{Albanian} data in \eqref{ex:key:10.17} seem to lend support to Grestenberger’s
...
(who bears dative\is{dative case}) the one being begged. In (\ref{ex:key:10.17}b), with the active
...
\ea\label{ex:key:10.17} \ili{Albanian}
    \ea
...
\ea\label{ex:key:10.18} \ili{Albanian}
    \ea
...
\ea\label{ex:key:10.19} \ili{Albanian}
    \ea
...
\ea\label{ex:key:10.20} \ili{Albanian}
    \ea[]{
...
The very same transitivization process as in (\ref{ex:key:10.20}b) is also
attested with pseudo-reflexives in \ili{Romance} and \ili{Germanic}, as illustrated in
\eqref{ex:key:10.21} for \ili{Italian} and \eqref{ex:key:10.22} for
...
\ea\label{ex:key:10.21} \ili{Italian}
    \ea[]{
...
\ea\label{ex:key:10.22} \ili{German}
    \ea[]{
	\gll    Ich  schäme  mich.\\
            I    shame  me/myself\\
    \glt    ‘I am ashamed of myself.’}
...
\ea\label{ex:key:10.23} \ili{Greek}
    \ea
...
\ea\label{ex:key:10.24} \ili{Greek}
    \sn
...
contradicted by data like those in \eqref{ex:key:10.25} for \ili{Italian} and
\eqref{ex:key:10.26} for \ili{German}, which specifically demonstrate that
unaccusative\is{unaccusativity}
syntax is not incompatible with such adverbs:

\ea\label{ex:key:10.25} \ili{Italian} \parencite{FolliHarley2006}
    \ea
...
\ea\label{ex:key:10.26} \ili{German}
    \sn
...
deponents comes from languages like Vedic and in some cases Ancient \ili{Greek},
...
self-explanatory: (\ref{ex:key:10.27}c) refers to the general passive\is{passive} voice,
...
tied to \citegen{Kallulli2007} system summarized in~\Cref{tb:10.1} below, with
...
(\ref{ex:key:10.22}b) across \ili{Albanian}, \ili{Italian} and \ili{German}, respectively, is
...
an instant, and says: “You might be right, it sounds \ili{Germanic}, but the moment
