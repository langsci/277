...
\abstract{This paper discusses Visser’s Generalization effects in light of the
    question of whether control involves a direct relation between the embedded
    PRO subject and a matrix controller, or an indirect relation mediated by a
    functional head\is{functional items} in the matrix clause. Based on certain case restrictions
and effects of additional \emph{by}-phrases, it is suggested that both types of
...
in \eqref{ex:key:23.1}, \ili{Dutch} and \ili{German} allow implicit matrix
...
\ea\label{ex:key:23.1}
    ...
    \ea\label{ex:key:23.1a} \ili{Dutch} \parencite[171, (8)]{vanUrk2013}\\
        \sn
    ...
    \ex\label{ex:key:23.1b} \ili{German} \parencite[171, (9a)]{vanUrk2013}\\
        \sn
...
to predicates like \emph{promise} in \eqref{ex:key:23.1} that combine with a dative\is{dative case} argument
...
This is shown in \eqref{ex:key:23.2} for \ili{Dutch} and \eqref{ex:key:23.3} for German. The (a) examples
...
\ea\label{ex:key:23.2} *\textsc{implicit.agent}  DP.\Acc{} $\to$ \Nom{}  V  [\textsubscript{\Inf{}}   PRO \textsubscript{ …}  ]\\
	\ea\label{ex:key:23.2a} \ili{Dutch} (P. Fenger, p.c.)\\
    \sn[]{
        ...
        \ex\label{ex:key:23.2b} \ili{Dutch} \parencite[171, (10b)]{vanUrk2013}\\
        \sn[*]{
		\gll  De leraren  werden overtuigd  om ze  te mogen kietelen.\\
...
\ea\label{ex:key:23.3} *\textsc{implicit.agent}  DP.\Acc{} $\to$ \Nom{}  V  [\textsubscript{\Inf{}}  PRO \textsubscript{…}  ]\\
	\ea\label{ex:key:23.3a} \ili{German} (personal knowledge)\\
        \sn[]{
    ...
    \ex\label{ex:key:23.3b} \ili{German} \parencite[171, \eqref{ex:key:23.10b}]{vanUrk2013}\\
        \sn[*]{
		\gll Der Lehrer  wurde  gebeten,  ihn  kitzeln  zu dürfen.\\
...
\z
%
Given that implicit control\is{control!implicit control} is, in principle, possible in these languages, a
direct control approach faces the question of how to distinguish between \eqref{ex:key:23.1}
and \eqref{ex:key:23.2}/(3) if implicit control\is{control!implicit control} is established as a direct dependency between
an implicit argument\is{implicit arguments} (e.g., \emph{pro}) and PRO. On the other hand, if
...
is illustrated in \eqref{ex:key:23.5}. I assume that implicit passive\is{passive} arguments are
syntactically represented as weak deficient pronouns, and, more specifically,
as φ-feature\is{φ-features} bundles without a D-layer  (see among many others
...
implicit passive\is{passive} arguments are not able to control (or bind) on their own.
...
\z
%
The failure of implicit control\is{control!implicit control} in \eqref{ex:key:23.2} and \eqref{ex:key:23.3}
...
create two conflicting referential dependencies. As a result, implicit control\is{control!implicit control}
is impossible and the only control relation that can be established in these
contexts is control by the nominative\is{nominative case} argument (which is in
general possible in passive\is{passive} contexts such as \eqref{ex:key:23.2}/(3); in the
specific examples above, it would be excluded due to the resulting \isi{binding}
...
\ea\label{ex:key:23.7}
	\ea\label{ex:key:23.7a} \ili{Dutch} (P. Fenger, p.c.)\\
    \sn
...
\ea\label{ex:key:23.8}
	\ea\label{ex:key:23.8a} \ili{Dutch} (P. Fenger, p.c.)\\
    \sn
...
\emph{by}-phrase Agent. As shown in \eqref{ex:key:23.10}, in both \ili{Dutch}
...
control and \isi{binding} relation becomes possible again.\largerpage[1]

\ea\label{ex:key:23.10}
	\ea\label{ex:key:23.10a} \ili{Dutch} (P. Fenger, p.c.)\\
        \sn[*]{
        \gll Mij  werd  beloofd  (om)  zijn / haar / hun  best  to doen.\\
    ...
    \ex\label{ex:key:23.10b} \ili{German}\\
        \sn[*]{
		\gll Mir  wurde  versprochen / angeboten,  sein / ihr  Bestes  zu geben.\\
...
\ea\label{ex:key:23.11} John just returned from a meeting with his boss. What happened?\\
    \ea\label{ex:key:23.11a}\ili{German}\\
    \sn
    ...
    \ex\label{ex:key:23.11b} \ili{German}\\
    \sn
